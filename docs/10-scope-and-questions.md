...
- **Player as caravan operator** — honest trader, fixer,
  ...
  news catches up.
- WebGL/PixiJS rendering, viewport-culled, using the generated
  fixed-isometric raster pipeline in [15 — Viewer](15-viewer.md).
  Multi-hex settlements draw as physical footprints; the viewer animates
  caravans and moving camps.
- Save/load.
...
| 33  | Cash circulation discipline                  | **Locked** — every actor kind that the schedule builder draws bids from must have a sustainable cash income channel (rents, tax, wages-in-coin, civic dividends, caravan/export profits, sales). Without it, comfort/status/capital markets go dormant on top of full stockpiles. See [08 — Money & Trade](08-money-and-trade.md) §"Cash circulation discipline"                                                                                                                                                                                                                |
| 34  | Physical inventory by settlement             | **Locked** — `Actor.stockpile` is keyed by `SettlementId` (`Map<SettlementId, Map<ResourceId, Quantity>>`). The same actor can hold inventory at multiple settlements but the pools are distinct: a recipe firing at building `b` only drains/credits `actor.stockpile.get(b.settlement)`. No hidden hand can satisfy markets in 18 settlements from one warehouse. See [11 — Politics & Ownership](11-politics-and-ownership.md)                                                                                                                                                                                                                   |
| 35  | Patron-client tribute (not pooled inventory) | **Locked** — client villages have their own `free_village` actor that owns the village's harvest. Patrons collect quarterly **coin tribute** (a fraction of village treasury) — they do not co-own the village's stockpile. The viewer reports physical stock at each settlement, not "ownership pool" abstractions. See [11 — Politics & Ownership](11-politics-and-ownership.md) §"Patron-client villages"                                                                                                                                                                                                                                    |
| 36  | Village / hamlet flow via villager caravans  | **Locked** — `free_village` and `hamlet_household` stewards can dispatch low-capacity villager pack caravans (2-4 mules, optional donkey, 1 drover + 1 guard) every 3 days when they have demand: exportable surplus, a home-learned import shortage, or hard-times resupply need. Accumulated treasury alone is not demand. There is no global rural slot pool; dispatch is limited by per-owner active cap, animals, starter rations, operating cash, known prices, and route economics. Villager caravans can carry a planned goal like "sell grain, buy tools": the shortage is known before departure, the purchase happens through a visited market, and owned imports unload into the steward's home stockpile on return. Distinct from patron-funded long-haul merchant trains; renders with its own unit visual class. See [06 — Caravans](06-caravans.md) §"NPC caravan AI" and [15 — Viewer](15-viewer.md) §"Units And Movement" |
| 37  | Caravans transact via local markets; owned cargo can be delivered home (v1.6/v1.9) | **Locked** — every caravan (international, long-haul, villager, replacement, player) buys and sells on the destination's CDA market via bid/ask. Cross-settlement trade does not directly mutate a third party's stockpile. Villager caravans may unload their own imported cargo into their owner's home stockpile after a market purchase; this is delivery of owned cargo, not a market bypass. The remaining `localTradePhase` implementation is same-hex only: pagus / dependent hamlets sharing one literal hex can clear at zero travel time, but distance ≥1 inter-settlement flow must use real caravan units. See [06 — Caravans](06-caravans.md) §"Caravan lifecycle", [08 — Money & Trade](08-money-and-trade.md) §"Caravans transact via local markets; owned cargo can be delivered home" |
| 38  | Per-actor information asymmetry on prices (v1.6) | **Locked** — every Actor carries `knownPrices: Map<SettlementId, MarketObservation>` where `MarketObservation` is a whole-ladder snapshot stamped with one `observedDay`. **One observation per (actor, settlement)**, not per-resource. Persistent city criers also carry their own `knownPrices` maps. Updated only by physical syncs: resident-presence (daily for actors at home), arrival, meeting (same-hex or same-settlement on same day, friendly/neutral only), guild-ledger, city-crier rural circuit, edge-hex. **Newer observedDay always wins atomically** on merge — older snapshot is discarded whole. **No deception channel**: hostile actors withhold sync, never feed false data. No source / provenance tracking. Observations older than 180 days are stale. No global oracle. See [06](06-caravans.md) §"Caravan information model" + [13](13-reputation-and-relationships.md) §"News-carrier price piggyback" |
| 39  | Outbound international ventures dispatched by patricians + guilds (v1.6) | **Locked** — outbound export caravans are not spawned by the old global schedule. Patrician families and merchant guilds at large/small cities evaluate route profit using their `knownPrices` maps and dispatch a venture when `expected_profit ≥ 3 × expected_transport_cost`. Transport cost = full operating cost (feed + wages + cart wear + bandit-loss exp + tolls), including the 20-tick off-map sojourn. Other domestic actor kinds do not dispatch outbound international trade. Inbound imports are separate edge-hub visits owned by synthetic `off_map_house` endpoints. See [06](06-caravans.md) §"International ventures" |
...
| 41  | Global market = infinite-demand sink at edge hexes (v1.6) | **Locked** — the off-map global market has **infinite buying and selling capacity** at the global reference price. The edge hex is the market venue. The old tiny edge-hub throttles are gone, but the implementation still evaluates edge trade on a daily cadence with high finite per-flow safety caps (`200` active imports, `300` active exports, `20` import spawns/day, `30` export spawns/day). There is no global active-caravan ceiling. What restrains volume is bounded actor information + finite treasuries + margin gates: outbound export ventures use the 3× transport-cost dispatch threshold; inbound imports spawn only when local scarcity creates positive landed margin. Per Pillar 8, every catalog resource has a global reference price. See [08](08-money-and-trade.md) §"The off-map global market" |
...
| 43  | All distance ≥1 inter-settlement trade goes through real caravan units (v1.6) | **Locked** — the abstract `localTradePhase` daily-pass that teleported goods between settlement pairs is deleted for distance ≥1. Every cross-settlement trade beyond same-hex is a real Caravan unit with a position, food consumption, ambush exposure, weather risk, and disease vector. Implemented tiers include villager pack caravans and standing merchant/international caravans; they share the SAME machinery and differ only in capacity, dispatcher, and route economics. Only same-hex coexistence (pagus + dependent hamlets sharing one literal hex) clears at 0-tick via the remaining same-hex `localTradePhase` / intra-hex market step. A villager pack caravan on a road is as ambush-exposed as a senatorial merchant train on the same road. See [06 — Caravans](06-caravans.md) §"Local trade between nearby settlements" |
| 44  | Recipe yields anchored to historical Roman per-event physics (v1.6) | **Locked** — every recipe's per-run output is justified against historical per-event capacity: a slaughter event produces (live weight × dressed yield × cure factor) salted meat, not an arbitrary "60 units"; a potter-day produces (~30-50 small pieces × avg weight) = ~1 unit of ware, not 4; a tailor-day produces 1 hand-sewn garment, not 2; a carpenter-day produces 0.5 furniture pieces (one chest/table per 2 days). Each recipe carries an inline justification comment derived from live-weight or skilled-day reference values. Calibrated to drain residual stockpile bloat (cities holding multi-year salted_meat / pottery / clothing / furniture stockpiles that production was outrunning the calibrated consumption rates). See [03 — Production](03-production.md) recipe catalog and the per-recipe `notes` field comments. |
| 45  | Off-map merchants have no permanent on-map presence (v1.9) | **Locked** — the `off_map_house` actor kind exists **only** as the per-edge-gate synthetic endpoint. No on-map settlement hosts an off-map merchant as a stockpile owner. Procgen-seeded warm-start caravans that previously assigned to misnamed "merchant houses" with `homeSettlement` set are now redistributed to `patrician_family` and `governor_office` owners. Inbound off-map caravans arrive at a destination city, sell imports, evaluate profitable return cargo, buy if profitable, ship back to the edge gate, and sell remaining cargo off-map. The caravan and its entire residual treasury are deleted at the edge — cash that returns off-map physically leaves our economy. There is no remittance to any on-map owner. This closes the foreign-trade loop and removes the 12.4M coin sink that v1.8 burn-in surfaced (9 procgen merchant houses hoarding without spending). See [06](06-caravans.md) §"Edge-hub inbound visits". |
| 46  | Mint exists only in the capital (v1.9) | **Locked** — only the capital city hosts a `mint` building. Other cities + towns + villages do not have local mints. Silver flows from mining sites to the capital because the capital's mint is the only buyer with derived-input demand for silver; the resulting `goods.coin` then circulates via taxation + trade. Previously every `large_city` got a mint, but procgen tended to place silver deposits in non-capital cities, leaving the capital's mint silver-starved while other cities' mints had no working setup. This change anchors monetary production in one well-known place and makes the silver-supply chain a real long-distance trade route. See [03 — Production](03-production.md) recipe `mint_coin` and [08 — Money & Trade](08-money-and-trade.md) §"Mint output flows to treasury". |
| 47  | Satiation-aware demand — INVESTIGATED, deferred (v1.9) | **Open.** The v1.8 burn-in surfaced city grain stockpiles growing 2k → 47k days of supply because subsistence demand kept bidding `population × per-capita-per-day` every day regardless of city granary holding. The intended fix — cap quantity demanded by `(own + communal_pool) / consumption_rate` against per-resource target days — cascaded badly in v1.9 burn-ins: suppressing the bid suppressed the same-tick consumption draw (consumption is wired through the market clearing, not via a separate stockpile drain). 3y famine deaths jumped from 770 (baseline) to 13-39k across multiple variants. Production-side tightening (smaller stock-target gate) similarly broke worker/wage flows. The user's intent ("100 years of grain → no trades") is correct but the current bid-driven consumption architecture isn't compatible with a hard demand cap. Future work: either (a) decouple consumption from bid-clearing so a 0 bid doesn't halt consumption, or (b) approach the same goal from a supply-side gate that throttles production at the seller. Both ideas surfaced second-order regressions and need fresh design. The supply-side stockpile-bloat problem in v1.8 remains a known issue. |

## Open design risks

- **Performance.** ~3,000–8,000 settlement entities × ~75
...
6. WebGL/PixiJS rendering of the hex map using the generated
   fixed-isometric raster pipeline, with multi-hex settlement footprints,
   viewport culling, and a single settlement panel.
