...
| 36  | Village / hamlet flow via villager caravans  | **Locked** — `free_village` and `hamlet_household` stewards can dispatch low-capacity villager pack caravans (2-4 mules, optional donkey, 1 drover + 1 guard) every 3 days when they have exportable surplus, accumulated import-trip treasury, or a hard-times resupply need. Surplus run / import trip / hard-times resupply are the three motivations; the normal caravan planner picks cargo + route each leg from known prices, bid depth, route cost, risk, and fallback scouting. Distinct from patron-funded long-haul merchant trains; renders with its own SVG glyph. See [06 — Caravans](06-caravans.md) §"NPC caravan AI", [15 — v1.5 cleanups](15-v1-5-cleanups.md) §C31 |
| 37  | Caravans transact via local markets, never stockpile (v1.6) | **Locked** — every caravan (international, long-haul, villager, replacement, player) buys and sells on the destination's CDA market via bid/ask. No direct stockpile transfer path exists. The remaining `localTradePhase` implementation is same-hex only: pagus / dependent hamlets sharing one literal hex can clear at zero travel time, but distance ≥1 inter-settlement flow must use real caravan units. See [06 — Caravans](06-caravans.md) §"Caravan lifecycle", [08 — Money & Trade](08-money-and-trade.md) §"Caravans transact via local markets, never stockpile" |
